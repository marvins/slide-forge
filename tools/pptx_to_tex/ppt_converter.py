...
class PresentationConverter:
    ...
    def _latex_to_pptx_fallback(self, input_path: str, output_path: str) -> str:
        ...
        # Extract preamble metadata for title slide
        meta = self._extract_latex_metadata(content)
        ...
        for frame_content in frames:
            # Handle special frames like titlepage before creating a slide
            if '\\titlepage' in frame_content:
                title_layout = prs.slide_layouts[0]  # Title slide layout
                title_slide = prs.slides.add_slide(title_layout)

                # Set title and subtitle with fallbacks
                title_text = meta.get("title") or "Presentation"
                subtitle_text = meta.get("subtitle") or meta.get("author") or ""

                # Set title
                if title_slide.shapes.title:
                    title_slide.shapes.title.text = title_text

                # Set subtitle (try different placeholder positions)
                subtitle_set = False
                ...
                # If subtitle wasn't set, try to add it as a text box
                if not subtitle_set:
                    from pptx.util import Inches
                    left = Inches(1)
                    top = Inches(2.5)
                ...
                image_paths = self._extract_latex_images(frame_content)
                for image_path in image_paths:
                    resolved_path = self._resolve_latex_image_path(input_path, image_path)
                    if resolved_path:
                        try:
                            # Position logo at bottom center of title slide
                            title_slide.shapes.add_picture(resolved_path, left=Inches(3), top=Inches(4), width=Inches(2))
                        except Exception:
                            logger.warning(f"Could not add image {image_path} to title slide")
            ...
            # Detect images
            image_paths = self._extract_latex_images(text_content)
            ...
            if final_text.strip() and len(slide.placeholders) > 1:
                content_placeholder = slide.placeholders[1]
                if has_bullets:
                    # Clear existing text and set up proper bullets
                    ...
                    for line in processed_lines:
                        if line.startswith("• "):
                            ...
                            p.text = line[2:].strip()  # Remove "• " prefix
                            p.level = 0
                            # Enable bullet
                            if hasattr(p, 'bullet'):
                                p.bullet = True
                    ...
                    content_placeholder.text = final_text
            ...
            for image_path in image_paths:
                ...
                if resolved_path:
                    try:
                        slide.shapes.add_picture(resolved_path, left=Inches(0.5), top=Inches(1.2), width=Inches(5.5))
                    except Exception:
                        logger.warning(f"Could not add image {image_path} to slide")
    ...
    def _extract_latex_metadata(self, content: str) -> Dict[str, str]:
        """Extract title metadata from LaTeX preamble."""
        def _match(cmd: str) -> Optional[str]:
            match = re.search(rf'\\{cmd}\{{(.*?)\}}', content, re.DOTALL)
            return match.group(1).strip() if match else None

        return {
            "title": _match("title") or "",
            "subtitle": _match("subtitle") or "",
            "author": _match("author") or "",
            "date": _match("date") or "",
            "institute": _match("institute") or "",
        }

    def _extract_latex_images(self, text: str) -> list:
        """Extract image paths from \\includegraphics commands."""
        images = []
        for match in re.finditer(r'\\includegraphics(?:\[[^\]]*\])?\{([^}]*)\}', text):
            images.append(match.group(1).strip())
        return images

    def _resolve_latex_image_path(self, tex_path: str, image_path: str) -> Optional[str]:
        """Resolve an image path relative to the LaTeX file."""
        tex_dir = Path(tex_path).parent
        candidate_paths = [
            tex_dir / image_path,
            tex_dir / "images" / image_path,
            Path("/Users/marvin/Desktop/Projects/Document-Experiments/latex/images") / Path(image_path).name,
        ]
